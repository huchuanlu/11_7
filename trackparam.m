...
switch (title)         
    ...
    case 'OneShopOneWait2cor';  
        p = [114,75,18,58,0.00];
        opt = struct('numsample',300, 'condenssig',0.25, 'ff',1, ...
                 'batchsize',5, 'affsig',[4,4,.000,.000,.000,.001]);
    case 'animal';  
        p = [350 40 100 70 0.0];
        opt = struct('numsample',300, 'condenssig',0.75, 'ff',0.99, ...
                 'batchsize',5, 'affsig',[16,16,.00,.00,.000,.000]);   
    case 'singer2';  
        p = [350 250 80 200 0.0];
        opt = struct('numsample',300, 'condenssig',0.75, 'ff',0.99, ...
                 'batchsize',5, 'affsig',[4,4,.00,.00,.000,.000]); 
    otherwise;  
